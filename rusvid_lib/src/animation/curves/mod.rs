...
macro_rules! generate_ease_struct {
    ...
    ($($x:ident),+ $(,)?) => (
        ...
        impl FunctionType {
            ...
            pub fn delta(&self, ease: crate::animation::EaseType, delta: f32) -> f32 {
                match ease {
                    crate::animation::EaseType::In => self.delta_ease_in(delta),
                    crate::animation::EaseType::Out => self.delta_ease_out(delta),
                    crate::animation::EaseType::InOut => self.delta_ease_in_out(delta),
                }
            }
        }
